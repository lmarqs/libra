...
- **NEVER** raise thrust/output limits (`kMaxThrottle`, `kPidOutLimit`, `kBaseThrottle`, ESC µs range) in `config.h` without explicit user confirmation. The props-off bench commands (`set motors_enabled` / `set motors_speed`) drive the ESCs directly at up to full throttle for calibration and **skip both the `kMaxThrottle` cap and the tilt failsafe** — but never change the balancing cap. Autonomous balancing keeps both safeguards.
