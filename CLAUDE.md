...
- **NEVER** widen the throttle band or raise the ESC µs range without explicit user confirmation. Three knobs set from `.env` define it: `LIBRA_THROTTLE_MIN` (`kMinThrottle`, hard floor — a motor never goes below it), `LIBRA_THROTTLE_MID` (`kBaseThrottle`, the hover point — set independently within the band), and `LIBRA_THROTTLE_MAX` (`kMaxThrottle`, hard ceiling — never beyond). MIN/MAX are the mixer's hard clamp; the PID authority (`kPidOutLimit`) is **derived** as the max headroom from MID — `max(MAX − MID, MID − MIN)` — so a full correction can reach the farther edge while the clamp holds the nearer one (a `static_assert` requires `MIN ≤ MID ≤ MAX`). Widening the band raises both thrust and authority. The committed default is a **zero band `[0.0, 0.0]` with hover 0** (2026-06-22): a fresh build is **inert** (never drives the motors) until the operator sets a real band for their motor. Fractions map onto the ESC's ~1000–2000 µs range (0.0 → 1000 µs idle, 1.0 → 2000 µs full). The props-off bench commands (`set motors_enabled` / `set motors_speed`, compiled in only with `LIBRA_BENCH_ENABLED=1`) drive the ESCs directly and **skip both the `kMaxThrottle` cap and the tilt failsafe** — but never change the band. Autonomous balancing keeps both safeguards (clamped to the band; tilt failsafe live).
