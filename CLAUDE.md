...
- **NEVER** raise thrust/output limits (`kMaxThrottle`, `kPidOutLimit`, `kBaseThrottle`, ESC µs range) in `config.h` without explicit user confirmation. The props-off bench commands (`t … !`, `cal`) may exceed `kMaxThrottle` *per command* for calibration only — they never change the balancing cap.
...
- _(2026-06-21)_ **All serial output goes through the Arduino `log_*` macros** (`log_e`/`log_w`/`log_i`/`log_d`), not raw `Serial.print*` — uniform `[time][level][file:line] func():` prefix, no `libra:`/`balance:`/`state:` text prefixes (file:line identifies the source). Levels: failures (`MPU6050 not found`, WiFi/httpd start) = `log_e`; invalid-command help = `log_w`; boot banner + arm/disarm + `state:` replies + AP join/leave = `log_i`; IMU bring-up stream = `log_d` (`>=4`). So `LIBRA_LOG_LEVEL` gates **everything** — below 3 the banner and command replies vanish (so `probe`/`banner` need `>=3`, the default); errors survive to 1. `Serial.setDebugOutput(true)` (set in `setup()`) is what routes `log_*` to USB-CDC and is now load-bearing for all output. `Serial` itself is input-only now (`pollSerial`). Don't reintroduce `Serial.print*` for diagnostics — `grep -rn 'Serial\.print' src` should stay empty.
- _(2026-06-21)_ **Bench / ESC-calibration commands (props off).** Serial `t <0..1>` drives both ESCs at a raw throttle, `cal`/`cal min` run the Afro Mini (SimonK/BLHeli) throttle-range teach, `run` resumes balancing, `x` soft-stops to idle. They set a `benchActive` override that `step()` checks **first** — it writes `benchThrottle` to both ESCs and returns, but still calls `web::publish(..., bench=true, ...)` so the web UI never shows a safe-looking state while the motors are live. The override bypasses the mixer's `kMaxThrottle` cap, so the gate is the pure host-tested `throttle::clampBenchThrottle` (`lib/throttle`): `t <v>` clamps to `kMaxThrottle`; a trailing **standalone** `!` (`t <v> !`, space required) is the explicit per-command confirmation to exceed it (still ≤ full scale). Calibration needs MAX present *at power-up*, but `escs.begin()` holds MIN for ~3 s at boot — so the ESC must be on a switch **separate from the board**: `cal` (streams MAX) → switch ESC on → `cal min` → `run`. Full procedure in docs/testing.md §7.
